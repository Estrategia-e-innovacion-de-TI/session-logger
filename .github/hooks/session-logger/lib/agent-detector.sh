...
# Detects whether a payload comes from GitHub Copilot or Claude Code
...
detect_agent_source() {
  ...
  local has_copilot_fields has_claude_fields
  ...
  if [[ "$has_claude_fields" == "true" ]]; then
    ...
    case "$event_type" in
      ...
      SessionStart|UserPromptSubmit|PreToolUse|PostToolUse|SessionEnd|ErrorOccurred)
        echo "claude_code"
        ;;
      # Copilot uses camelCase
      sessionStart|userPromptSubmitted|preToolUse|postToolUse|sessionEnd|errorOccurred)
        echo "github_copilot"
        ;;
      *)
        # Final fallback: if source env var suggests copilot, assume copilot
