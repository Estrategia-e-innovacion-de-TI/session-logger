...
function Main {
    $loggerSource = Get-EnvOrDefault -Name 'CURSOR_SESSION_LOGGER_SOURCE' -Default 'github_cursor_hook'
    $rawLokiEnabled = [Environment]::GetEnvironmentVariable('CURSOR_SESSION_LOGGER_LOKI_ENABLED')
    $rawLokiEndpoint = [Environment]::GetEnvironmentVariable('CURSOR_SESSION_LOGGER_LOKI_ENDPOINT')
    $rawLokiTenantId = [Environment]::GetEnvironmentVariable('CURSOR_SESSION_LOGGER_LOKI_TENANT_ID')
    if ([string]::IsNullOrWhiteSpace($rawLokiEnabled)) {
        ...
        $lokiEnabled = Get-EnvBool -Name 'CURSOR_SESSION_LOGGER_LOKI_ENABLED' -Default $false
    }
    $lokiEndpoint = Get-EnvOrDefault -Name 'CURSOR_SESSION_LOGGER_LOKI_ENDPOINT' -Default 'http://localhost:3100/loki/api/v1/push'
    $lokiTenantId = Get-EnvOrDefault -Name 'CURSOR_SESSION_LOGGER_LOKI_TENANT_ID' -Default ''
    $rawOtlpEnabled = [Environment]::GetEnvironmentVariable('CURSOR_SESSION_LOGGER_OTLP_ENABLED')
    $rawOtlpEndpoint = [Environment]::GetEnvironmentVariable('CURSOR_SESSION_LOGGER_OTLP_ENDPOINT')
    if ([string]::IsNullOrWhiteSpace($rawOtlpEnabled)) {
        $otlpEnabled = -not [string]::IsNullOrWhiteSpace($rawOtlpEndpoint)
    }
    else {
        $otlpEnabled = Get-EnvBool -Name 'CURSOR_SESSION_LOGGER_OTLP_ENABLED' -Default $false
    }
    $otlpEndpoint = Get-EnvOrDefault -Name 'CURSOR_SESSION_LOGGER_OTLP_ENDPOINT' -Default 'http://localhost:4318'
    $timeoutSeconds = [int](Get-EnvOrDefault -Name 'CURSOR_SESSION_LOGGER_TIMEOUT_SECONDS' -Default '2')
    $actor = Get-EnvOrDefault -Name 'CURSOR_SESSION_LOGGER_ACTOR' -Default $env:USERNAME
    ...
    $modeRaw = Get-First -Object $payload -Paths @(@('mode'), @('chat_mode'), @('chatMode'), @('cursor_mode'), @('cursorMode'), @('invocation','mode'), @('payload','mode'), @('payload','chat_mode'), @('payload','chatMode'), @('request','mode'), @('request','chat_mode'), @('request','chatMode'))
    ...
    $pluginDetected = ($null -ne $toolNameCanonical -and $toolNameCanonical -match '^(vscode_|extension_|plugin_|plugin\.|cursor\.)')
    ...
    if ($attempted -eq 0) {
        Write-Error 'no_observability_transports_enabled: enable CURSOR_SESSION_LOGGER_LOKI_ENABLED and/or CURSOR_SESSION_LOGGER_OTLP_ENABLED'
        exit 1
    }
